...
import { BaseQuery, NewShopRequestBody, SearchRequestQuery, SearchShopRequestQuery, ShopBaseQuery } from "../types/types.js";
...
export const newShop = TryCatch(
    async(req:Request<{},{},NewShopRequestBody>,res,next) =>{
...
export const getShopsByAddress = TryCatch(
    async(req,res,next) =>{
        let shops;
...
export const getlatestShops = TryCatch(
    async(req,res,next) =>{
        let shops;

        if(myCache.has("latest-shops"))
        shops = JSON.parse(myCache.get("latest-shops") as string)
...
export const getAllShopCategories = TryCatch(
    async(req,res,next) =>{

        let categories;

        if(myCache.has("shop-categories")){
            categories = JSON.parse(myCache.get("shop-categories") as string);
        }
        else{
...
export const getAdminShops = TryCatch(
    async(req,res,next) =>{

        let shops;

        if(myCache.has("all-shops"))
        shops = JSON.parse(myCache.get("all-shops") as string)
...
export const getSingleShop = TryCatch(
    async(req,res,next) =>{
        ...
        if(myCache.has(`shop-${id}`)){
            shop = JSON.parse(myCache.get(`shop-${id}`) as string)
        }
...
export const getSingleShopProducts = TryCatch(
    async(req,res,next) =>{
...
export const updateShop = TryCatch(
    async(req,res,next) =>{
        ...
        if(deliveryAddress) shop.deliveryAddress=deliveryAddress;
...
export const deleteShop = TryCatch(
    async(req,res,next) =>{
...
export const searchAllshops = TryCatch(
    async(req:Request<{},{},{},SearchShopRequestQuery>,
        res,
        next) =>{
